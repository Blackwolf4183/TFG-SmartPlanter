...
async def create_device(clientId, password):
    ...
    if device and device.data:  # Check if device data exists
        ...
        return {"message": "Device created successfully", "device": response_device}
    else:
        raise HTTPException(status_code=500, detail="Device could not be created")
...
async def link_user_to_device(device_id, device_password, user):
    ...
    if(len(device.data) == 0):
        ...
        raise HTTPException(status_code=400, detail="No device registered with such id")
    
    #Check if password is incorrect
    if(not verify_password(device_password, device.data[0]["password"])):
        raise HTTPException(status_code=401, detail="Incorrect password for device")
    ...
    if(len(exiting_userdevice.data) > 0):
        raise HTTPException(status_code=400, detail="User has already one device linked")
    ...
    if not user_device or not user_device.data:  # Check if device data exists
        raise HTTPException(status_code=500, detail="Device could not be linked with user")